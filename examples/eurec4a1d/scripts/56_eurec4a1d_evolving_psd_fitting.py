...
config_file_path = path2CLEO / "examples/eurec4a1d/default_config/eurec4a1d_config_evolving.yaml"
constants_file_path = path2CLEO / "libs/cleoconstants.hpp"
binary_path = path2build / "bin/"
share_path = path2build / "share/"
cloud_observation_file_path = path2CLEO / "examples/eurec4a1d/default_config/cloud_observation.yaml"
...
data_dir = Path(r'C:\Users\Niebaum\Documents\Repositories\sdm-eurec4a\data\model\inputv4.0')

# Load the parameters datasets
psd_parameters = xr.open_dataset(data_dir / "particle_size_distribution_parameters.nc")
potential_temperature_parameters = xr.open_dataset(data_dir / "potential_temperature_parameters.nc")
relative_humidity_parameters = xr.open_dataset(data_dir / "relative_humidity_parameters.nc")
...
updated_configfile["coupled_dynamics"] = dict(
    ...
    press=str(data_dir / "share/eurec4a1d_dimlessthermo_press.dat"),  # binary filename for pressure
    temp=str(data_dir / "share/eurec4a1d_dimlessthermo_temp.dat"),  # binary filename for temperature
    qvap=str(data_dir / "share/eurec4a1d_dimlessthermo_qvap.dat"),  # binary filename for vapour mixing ratio
    qcond=str(data_dir / "share/eurec4a1d_dimlessthermo_qcond.dat"),  # binary filename for liquid mixing ratio
    wvel=str(data_dir / "share/eurec4a1d_dimlessthermo_wvel.dat"),  # binary filename for vertical (coord3) velocity
    thermo=str(data_dir / "share/eurec4a1d_dimlessthermo.dat"),  # binary filename for thermodynamic profiles
)
updated_configfile["inputfiles"] = dict(
    ...
    gridfilename=str(data_dir / "share/eurec4a1d_ddimlessGBxboundaries.dat"),  # binary filename for initialisation of GBxs / GbxMaps
)
...
ds_cleo['mass_size_distribution'] = msd_from_psd_dataarray(ds_cleo['particle_size_distribution'])
...
for _ax, variable in zip(axs, ['multiplicities', 'particle_size_distribution', 'mass_size_distribution']):
...
slr = thermogen.SplittedLapseRates(
    configfile = config_file_path,
    constsfile = constants_file_path,
    cloud_base_height = relative_humidity_parameters['x_split'].sel(cloud_id = cloud_id).values,
    pressure_0 = pressure_parameters['f_0'].sel(cloud_id = cloud_id).values,
    potential_temperature_0 = potential_temperature_parameters['f_0'].sel(cloud_id = cloud_id).values,
    relative_humidity_0 = relative_humidity_parameters['f_0'].sel(cloud_id = cloud_id).values,
...
thermodata = rthermo.get_thermodynamics_from_thermofile(thermo_file_path, ndims, inputs=inputs_dict)
...
if isfigures[0]:
    rgrid.plot_gridboxboundaries(constants_file_path, gridfile_path, fig_dir, isfigures[1])
...
dln_NUMCONC = probdists.DoubleLogNormal(
    **psd_parameters_linear_space
)
